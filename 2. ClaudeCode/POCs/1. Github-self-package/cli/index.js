...
// rgcodetoolkit — install Claude Code skills from the rgcodetoolkit collection.
//
//   npx rgcodetoolkit install-skill dev-team-lead            → ./dev-team-lead/   (just files)
//   npx rgcodetoolkit install-skill dev-team-lead --claude   → ~/.claude/skills/ (opt-in)
//
...
const HELP = `rgcodetoolkit — install Claude Code skills from the rgcodetoolkit collection.

Usage:
  rgcodetoolkit install-skill <skill> [--claude] [--force]   Download a skill
  rgcodetoolkit list                                          List skills bundled in this package
  rgcodetoolkit help                                          Show this help
...
Examples:
  npx rgcodetoolkit install-skill dev-team-lead                 → ./dev-team-lead/
  npx rgcodetoolkit install-skill dev-team-lead --claude        → ~/.claude/skills/dev-team-lead/
  npx rgcodetoolkit install-skill dev-team-lead --force         overwrite ./dev-team-lead/
...
function installSkill(skill, opts) {
  if (!skill) fail('missing skill name. Try: rgcodetoolkit list');
...
function listSkills() {
  ...
  console.log('Bundled skills (install with: rgcodetoolkit install-skill <name>):\n');
...
function main(argv) {
  ...
  if (command !== 'install-skill') {
    fail(`unknown command '${command}'. Try: rgcodetoolkit help`);
  }
  ...
  for (const a of rest) {
    ...
    else if (a.startsWith('--')) fail(`unknown option '${a}'. Try: rgcodetoolkit help`);
    else if (a === '-h' || a === '--help') {
      console.log(HELP);
      return;
